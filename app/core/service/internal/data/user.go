...
import (
	...
	"github.com/go-kratos/beer-shop/app/core/service/internal/data/ent"
	"github.com/go-kratos/beer-shop/app/core/service/internal/data/ent/user"
	"github.com/go-kratos/beer-shop/app/core/service/internal/pkg/util"
	"time"
...
func (r *userRepo) serUserCache(ctx context.Context, user *ent.User, key string) {
	...
	if err != nil {
		r.log.Errorf("fail to set core cache:json.Marshal(%v) error(%v)", user, err)
	}
	err = r.data.redisCli.Set(ctx, key, string(marshal), time.Minute*30).Err()
	if err != nil {
		r.log.Errorf("fail to set core cache:redis.Set(%v) error(%v)", user, err)
	}
}
